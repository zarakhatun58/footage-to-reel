...
export const GoogleLoginUnified = ({ onClose }: { onClose?: () => void }) => {
  ...
  const handleLogin = () => {
    const scope = encodeURIComponent("openid profile email https://www.googleapis.com/auth/photoslibrary.readonly");
    // access_type=offline + prompt=consent ensures refresh token is issued
